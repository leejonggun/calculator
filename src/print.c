...
void list_print(token_t *root) {
	...
	while (Debug->next != NULL) {
		if (Debug->tt == OPEN || Debug->tt == CLOSE) {
			printf("tt = %s, status = %d, counter = %d, str = %s\n", type_name[Debug->tt], Debug->status, Debug->counter, Debug->str);
		} else {
			printf("tt = %s, status = %d, str_size = %d, str = %s\n", type_name[Debug->tt], Debug->status, Debug->str_size, Debug->str);
		}
		Debug = Debug->next;
	}
	printf("tt = %s, status = %d, counter = %d, str = %s\n", type_name[Debug->tt], Debug->status, Debug->counter, Debug->str);
